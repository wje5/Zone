...
public class LogRecvPack extends Log {
	private StorageWrapper items;
	private int packId, packTime;
	private List<SerialNumber> path;
	private SerialNumber start, end;

	public LogRecvPack(int id, int packId, StorageWrapper items, SerialNumber start, SerialNumber end,
			List<SerialNumber> path, int packTime) {
		super(Level.DEBUG, Type.RECVPACK, id);
		this.packId = packId;
	...
	public LogRecvPack(NBTTagCompound tag) {
		super(tag);
	}
	...
	public FormattedLog format() {
		return new FormattedLog(getTime(), getLevel(), "log.recv_pack", new LogComponentPack(packId, getItems()), start,
