...
public class SubscreenWizardSetAdminPassword extends Subscreen {
	...
	public SubscreenWizardSetAdminPassword(IHasSubscreen parent) {
		this(parent, getDisplayWidth() / 2 - 150, getDisplayHeight() / 2 - 100);
	}

	public SubscreenWizardSetAdminPassword(IHasSubscreen parent, int x, int y) {
		...
		components.add(new TextButton(this, this.x + 242 - w, this.y + 175, s, () -> {
			if (box1.text.length() == 8 && box1.text.equals(box2.text)) {
				parent.removeScreen(SubscreenWizardSetAdminPassword.this);
	...
	public void doRenderBackground(int mouseX, int mouseY) {
		...
		fr.drawString(I18n.format("sphinx.set_admin_password"), x + 30, y + 35, 0xFF1ECCDE);
		fr.drawString(I18n.format("sphinx.confirm_admin_password"), x + 30, y + 75, 0xFF1ECCDE);
		if (box1.text.length() > 0 && box1.text.length() != 8) {
			fr.drawString(I18n.format("sphinx.password_length_error"), x + 30, y + 115, 0xFFDA2D2D);
		} else if (box1.text.length() > 0 && box2.text.length() > 0 && !box1.text.equals(box2.text)) {
			fr.drawString(I18n.format("sphinx.confirm_password_error"), x + 30, y + 115, 0xFFDA2D2D);
		}
	}
}
