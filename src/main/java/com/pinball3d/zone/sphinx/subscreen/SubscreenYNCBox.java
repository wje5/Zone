...
public class SubscreenYNCBox extends Subscreen {
	...
	public SubscreenYNCBox(IHasSubscreen parent, int x, int y, String title, String text, Runnable yes, Runnable no) {
		...
		addComponent(new TextButton(this, this.x + 15, this.y + 80, I18n.format("sphinx.yes"), () -> {
			parent.removeScreen(SubscreenYNCBox.this);
			if (yes != null) {
				yes.run();
			}
		}));
		addComponent(new TextButton(this, this.x + 55, this.y + 80, I18n.format("sphinx.no"), () -> {
			parent.removeScreen(SubscreenYNCBox.this);
			if (no != null) {
				no.run();
			}
		}));
		addComponent(new TextButton(this, this.x + 95, this.y + 80, I18n.format("sphinx.cancel"), () -> {
			parent.removeScreen(SubscreenYNCBox.this);
		}));
		this.title = title;
		this.text = text;
