...
public class TEPoweredPiston extends TileEntityPiston {
	...
	public TEPoweredPiston() {
	}

	public TEPoweredPiston(IBlockState pistonStateIn, EnumFacing pistonFacingIn, boolean extendingIn,
	...
	public AxisAlignedBB getAABB(IBlockAccess p_184319_1_, BlockPos p_184319_2_, float p_184319_3_) {
		...
		return iblockstate.getBoundingBox(p_184319_1_, p_184319_2_).offset(
				p_184319_3_ * this.pistonFacing.getFrontOffsetX(), p_184319_3_ * this.pistonFacing.getFrontOffsetY(),
		...
		return !this.isExtending() && this.shouldPistonHeadBeRendered()
				? Blocks.PISTON_HEAD.getDefaultState()
						.withProperty(BlockPistonExtension.TYPE,
								this.pistonState.getBlock() == BlockLoader.powered_piston_sticky
	...
	private void moveCollidedEntities(float p_184322_1_) {
		...
		this.getCollisionRelatedBlockState().addCollisionBoxToList(this.world, BlockPos.ORIGIN,
				new AxisAlignedBB(BlockPos.ORIGIN), list, (Entity) null, true);
		...
		if (!list.isEmpty()) {
			...
			List<Entity> list1 = this.world.getEntitiesWithinAABBExcludingEntity((Entity) null,
					this.getMovementArea(axisalignedbb, enumfacing, d0).union(axisalignedbb));
				...
				for (int i = 0; i < list1.size(); ++i) {
					...
					if (entity.getPushReaction() != EnumPushReaction.IGNORE) {
						...
						if (d1 > 0.0D) {
							...
							entity.move(MoverType.PISTON, d1 * enumfacing.getFrontOffsetX(),
									d1 * enumfacing.getFrontOffsetY(), d1 * enumfacing.getFrontOffsetZ());
	...
	private AxisAlignedBB moveByPositionAndProgress(AxisAlignedBB p_190607_1_) {
		...
		return p_190607_1_.offset(this.pos.getX() + d0 * this.pistonFacing.getFrontOffsetX(),
				this.pos.getY() + d0 * this.pistonFacing.getFrontOffsetY(),
				this.pos.getZ() + d0 * this.pistonFacing.getFrontOffsetZ());
	}
	...
	private void fixEntityWithinPistonBase(Entity p_190605_1_, EnumFacing p_190605_2_, double p_190605_3_) {
		...
		if (axisalignedbb.intersects(axisalignedbb1)) {
			...
			if (Math.abs(d0 - d1) < 0.01D) {
				...
				p_190605_1_.move(MoverType.PISTON, d0 * enumfacing.getFrontOffsetX(), d0 * enumfacing.getFrontOffsetY(),
						d0 * enumfacing.getFrontOffsetZ());
				MOVING_ENTITY.set(null);
			}
		}
	}
	...
	public void clearPistonTileEntity() {
		if (this.lastProgress < 1.0F && this.world != null) {
			...
			if (this.world.getBlockState(this.pos).getBlock() == BlockLoader.powered_piston_moving) {
	...
	public void addCollissionAABBs(World p_190609_1_, BlockPos p_190609_2_, AxisAlignedBB p_190609_3_,
		...
		if (this.progress >= 1.0D
			...
			if (this.shouldPistonHeadBeRendered()) {
				iblockstate = BlockLoader.powered_piston_head.getDefaultState()
