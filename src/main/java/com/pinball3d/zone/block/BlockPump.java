...
public class BlockPump extends BlockContainer {
	...
	public BlockPump(boolean burning) {
		super(Material.IRON);
		setHardness(200.0F);
		setResistance(5000.0F);
		setLightLevel(burning ? 1F : 0F);
		setRegistryName("zone:pump" + (burning ? "_light" : ""));
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (!worldIn.isRemote) {
			playerIn.openGui(Zone.instance, GuiElementLoader.PUMP, worldIn, pos.getX(), pos.getY(), pos.getZ());
		}
		return true;
	}
	...
	public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
		if (!keepInventory) {
			TEPump te = (TEPump) worldIn.getTileEntity(pos);
	...
	public static void setState(boolean active, World worldIn, BlockPos pos) {
		...
		if (active) {
			worldIn.setBlockState(pos,
					BlockLoader.pump_light.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)),
			...
			worldIn.setBlockState(pos,
					BlockLoader.pump.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)),
					Constants.BlockFlags.DEFAULT_AND_RERENDER);
			worldIn.setBlockState(pos,
					BlockLoader.pump.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)),
					Constants.BlockFlags.DEFAULT_AND_RERENDER);
	...
	public ItemStack getItem(World worldIn, BlockPos pos, IBlockState state) {
		return new ItemStack(BlockLoader.pump);
	}

	@Override
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
		return Item.getItemFromBlock(BlockLoader.pump);
	}
