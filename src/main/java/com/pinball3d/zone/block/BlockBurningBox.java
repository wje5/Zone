...
public class BlockBurningBox extends BlockContainer {
	...
	public BlockBurningBox(boolean burning) {
		super(Material.ROCK);
		setHardness(2.0F);
		setLightLevel(burning ? 1F : 0F);
		setRegistryName("zone:burning_box" + (burning ? "_light" : ""));
		setUnlocalizedName("burning_box");
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (!worldIn.isRemote) {
			playerIn.openGui(Zone.instance, GuiElementLoader.BURNING_BOX, worldIn, pos.getX(), pos.getY(), pos.getZ());
		}
		return true;
	}
	...
	public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
		if (!keepInventory) {
			TEBurningBox te = (TEBurningBox) worldIn.getTileEntity(pos);
	...
	public ItemStack getItem(World worldIn, BlockPos pos, IBlockState state) {
		return new ItemStack(BlockLoader.burning_box);
	}

	@Override
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
		return Item.getItemFromBlock(BlockLoader.burning_box);
	}

	@Override
	public TileEntity createNewTileEntity(World worldIn, int meta) {
		return new TEBurningBox();
	}
}
