...
import com.pinball3d.zone.tileentity.TELathe;
...
public class BlockLathe extends BlockContainer {
	public static final PropertyDirection FACING = PropertyDirection.create("facing", EnumFacing.Plane.HORIZONTAL);
	private static boolean keepInventory;

	public BlockLathe(boolean burning) {
		super(Material.IRON);
		setHardness(200.0F);
		setResistance(5000.0F);
		setLightLevel(burning ? 1F : 0F);
		setRegistryName("zone:lathe" + (burning ? "_light" : ""));
		setUnlocalizedName("lathe");
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (!worldIn.isRemote) {
			playerIn.openGui(Zone.instance, GuiElementLoader.LATHE, worldIn, pos.getX(), pos.getY(), pos.getZ());
		}
		return true;
	}
	...
	public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
		if (!keepInventory) {
			TELathe te = (TELathe) worldIn.getTileEntity(pos);
	...
	public static void setState(boolean active, World worldIn, BlockPos pos) {
		...
		if (active) {
			worldIn.setBlockState(pos,
					BlockLoader.lathe_light.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)),
			...
			worldIn.setBlockState(pos,
					BlockLoader.lathe.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)),
					Constants.BlockFlags.DEFAULT_AND_RERENDER);
			worldIn.setBlockState(pos,
					BlockLoader.lathe.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)),
					Constants.BlockFlags.DEFAULT_AND_RERENDER);
	...
	public ItemStack getItem(World worldIn, BlockPos pos, IBlockState state) {
		return new ItemStack(BlockLoader.lathe);
	}

	@Override
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
		return Item.getItemFromBlock(BlockLoader.lathe);
	}
