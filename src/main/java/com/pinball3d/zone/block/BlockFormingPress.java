...
public class BlockFormingPress extends BlockContainer {
	public static final PropertyDirection FACING = PropertyDirection.create("facing", EnumFacing.Plane.HORIZONTAL);
	private static boolean keepInventory;

	public BlockFormingPress(boolean burning) {
		super(Material.IRON);
		setHardness(200.0F);
		setResistance(5000.0F);
		setLightLevel(burning ? 1F : 0F);
		setRegistryName("zone:forming_press" + (burning ? "_light" : ""));
		setUnlocalizedName("forming_press");
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (!worldIn.isRemote) {
			playerIn.openGui(Zone.instance, GuiElementLoader.FORMING_PRESS, worldIn, pos.getX(), pos.getY(),
					pos.getZ());
		}
		return true;
	}
	...
	public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
		if (!keepInventory) {
			TEFormingPress te = (TEFormingPress) worldIn.getTileEntity(pos);
	...
	public static void setState(boolean active, World worldIn, BlockPos pos) {
		...
		if (active) {
			worldIn.setBlockState(pos, BlockLoader.forming_press_light.getDefaultState().withProperty(FACING,
					iblockstate.getValue(FACING)), Constants.BlockFlags.DEFAULT_AND_RERENDER);
			worldIn.setBlockState(pos, BlockLoader.forming_press_light.getDefaultState().withProperty(FACING,
					iblockstate.getValue(FACING)), Constants.BlockFlags.DEFAULT_AND_RERENDER);
		} else {
			worldIn.setBlockState(pos,
					BlockLoader.forming_press.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)),
	...
	public ItemStack getItem(World worldIn, BlockPos pos, IBlockState state) {
		return new ItemStack(BlockLoader.forming_press);
	}

	@Override
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
		return Item.getItemFromBlock(BlockLoader.forming_press);
	}

	@Override
	public TileEntity createNewTileEntity(World worldIn, int meta) {
		return new TEFormingPress();
	}
}
