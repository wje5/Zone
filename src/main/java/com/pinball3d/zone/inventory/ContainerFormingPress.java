...
public class ContainerFormingPress extends Container {
	private IItemHandler energy, input, output;
	private int tick, totalTick, energyTick;
	protected TEFormingPress tileEntity;

	public ContainerFormingPress(EntityPlayer player, TileEntity tileEntity) {
		this.tileEntity = (TEFormingPress) tileEntity;
		...
		addSlotToContainer(new SlotItemHandler(input, 0, 47, 17));
		addSlotToContainer(new SlotItemHandler(input, 1, 65, 17));
...
//		energyTick = tileEntity.getEnergyTick();
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
		...
		if (index <= 3) {
			...
			if (!isMerged) {
				isMerged = mergeItemStack(newStack, 31, 40, false);
			}
		} else if (index >= 31) {
			isMerged = mergeItemStack(newStack, 0, 3, false);
			if (!isMerged) {
				isMerged = mergeItemStack(newStack, 4, 31, false);
			}
		}
		if (!isMerged) {
			return ItemStack.EMPTY;
