...
public class ContainerCrystallizer extends ContainerTieredMachine {
	private IItemHandler battery, input, output;
	private int tick, totalTick;

	public ContainerCrystallizer(EntityPlayer player, TECrystallizer tileEntity) {
	...
	public void detectAndSendChanges() {
		...
		if (!tileEntity.getWorld().isRemote) {
			tick = ((TECrystallizer) tileEntity).getTick();
			totalTick = ((TECrystallizer) tileEntity).getTotalTick();
		}
